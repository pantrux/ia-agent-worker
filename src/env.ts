...
export interface Env {
  ...
  /** Slug del custom provider (sin prefijo `custom-`). Si está definido, el cliente usa la ruta específica `…/custom-{slug}/inference` y el campo `model` es el id del catálogo (p. ej. `openai/gpt-5-mini`). El `base_url` del proveedor en Cloudflare debe ser `https://models.github.ai`. */
