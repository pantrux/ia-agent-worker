...
| B2b | Export / agregación automática de métricas Worker (p95, histórico) | *(PR en la rama `cursor-cloud/b2b-worker-metrics-pan9-1223`)* | En curso | [PAN-9](https://linear.app/pantrux/issue/PAN-9/b2b-export-o-agregacion-automatica-de-metricas-worker-p95-historico). Decisión + pipeline: [B2b-worker-access-metrics-export.md](./B2b-worker-access-metrics-export.md); `logpush` en `wrangler.toml`; agregador `scripts/aggregate-ia-agent-access.mjs`. |
...
| 2026-05-16 | **B2b / PAN-9:** documento [B2b-worker-access-metrics-export.md](./B2b-worker-access-metrics-export.md), agregador NDJSON + CI, `logpush = true` en Wrangler; guía B2 §2.4 enlazada. |
