...
| C3.2 | Reducir latencia hasta tarjeta HITL en `/demo` | rama `feat-pan-40-reducir-latencia-hitl` | En curso | [PAN-40](https://linear.app/pantrux/issue/PAN-40). Análisis cuantitativo en prod (2026-05-25): 31,7 s end-to-end, 3 llamadas Copilot en serie (≈18,7 s) dominan el coste. Quick Win 1 implementado: nodo `terminal_reply` y edge condicional `tools → [model | terminal_reply]` saltan el segundo `model_node` LLM cuando el `ToolMessage` es terminal (`Customer not found`, denegación, error JSON), ahorrando ≈9 s sin cambiar la lógica HITL. Mantiene `reply_reset` + `reply_delta` para UX streaming. |
