...
| B3 | Dataset mínimo + eval en LangSmith (snapshot en repo + CI opcional) | — | En curso | Estrategia: fuente de verdad del dataset en **LangSmith**; `evals/dataset-v0.json` + `npm run langsmith:dataset:sync` / `langsmith:eval`; umbral `EVAL_MIN_MEAN_SCORE`. Ver [B3-langsmith-llmops.md](./B3-langsmith-llmops.md). |
