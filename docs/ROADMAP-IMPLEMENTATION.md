...
| B2b | Export / agregación automática de métricas Worker (p95, histórico) | — | Pendiente | Seguimiento B2: [PAN-9](https://linear.app/pantrux/issue/PAN-9/b2b-export-o-agregacion-automatica-de-metricas-worker-p95-historico). Logpush u otro pipeline sobre logs `ia_agent_access`; etiquetas Linear **Feature** + **Área → Observabilidad**. |
