...
| B1b | AI Gateway: ruta **provider-specific** para custom provider (GitHub Models) | *PR abierto desde rama `fix/b1-ai-gateway-provider-specific`* | En curso | El endpoint Unified `/compat` con `model: custom-{slug}/…` podía resolver a paths upstream inválidos (p. ej. 404). Con `AI_GATEWAY_PROVIDER_SLUG`, el cliente usa `…/v1/{cuenta}/{gateway}/custom-{slug}`; el SDK añade `/chat/completions` → upstream `https://models.github.ai/inference/chat/completions`. README + `provision-ai-gateway.mjs` alineados. Opt-in `EXPOSE_CHAT_ERROR` en `index.ts` / `wrangler.toml` para depuración de 500 (apagar en prod). |
...
| 2026-05-13 | **B1b en curso:** fix provider-specific (`src/ai-gateway.ts`), documentación README/script, `EXPOSE_CHAT_ERROR` opt-in; trazabilidad en esta tabla. Tras merge del PR: B1b → **Ejecutado** y sustituir el placeholder de la columna PR por el enlace real. |
