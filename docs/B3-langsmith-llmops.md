...
- **GitHub Actions** actúa como diagnóstico y, opcionalmente, como puerta: si existen credenciales, ejecuta sincronización + experimento. El score puede quedar como **diagnóstico** en PR y solo convertirse en **gate duro** si activas `LANGSMITH_EVAL_ENFORCE=true`.
...
| `LANGSMITH_EVAL_ENFORCE` | No | Si es `true`/`1`, un score por debajo del umbral hace fallar el script. Vacío/`false` => deja resumen diagnóstico y exit 0. |
...
`eval_pass = 1` solo si se cumple todo lo anterior. La media se compara con `EVAL_MIN_MEAN_SCORE` (por defecto `0.875`).

- Si `LANGSMITH_EVAL_ENFORCE=true`, score bajo => **falla** el job.
- Si `LANGSMITH_EVAL_ENFORCE` está vacío o `false`, score bajo => **solo diagnóstico**: el script imprime un resumen agregado (`httpOk_false`, `threadOk_false`, `replyOk_false`, etc.) y deja el experimento accesible en LangSmith.
