...
- [x] Variable `DEPLOYMENT_ENV` en `wrangler.toml` por entorno para filtrar runs (metadata `deployment`) — [PAN-29](https://linear.app/pantrux/issue/PAN-29); definida `production` en `[vars]` y `preview` en `[env.preview.vars]`; verificada en runtime con un smoke prod cuya traza LangSmith expone `metadata.deployment="production"` y tag `env:production`. Preview: el binding está cargado por wrangler; el envío real de trazas en preview sigue el roadmap LangSmith (proyecto separado + secretos `LANGSMITH_*` específicos de preview, ver primer ítem de esta lista).
